...
% outline notes render in gray italics; delete \outline definition to hide them
\newcommand{\outline}[1]{\medskip\noindent{\small\color{gray}\textit{[#1]}}\medskip}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \todo{extend: arithmetic ($A{-}B{+}C$); generalization to unseen combinations.}
\end{itemize}
...
training}. The cosine similarity between a layer's true and predictable gradient is
$\approx0.99$ at initialization, $0.6$--$0.8$ at mid-training, and near zero at
convergence---gradients become noise as the loss flattens. This bounds any
synthetic-gradient method to the early, high-loss regime and explains why such methods
help least exactly when one most wants a shortcut. The broader lesson---that
